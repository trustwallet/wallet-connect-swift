...
class WCSessionManagerTests: XCTestCase {
    ...
    override func setUp() {
        ...
        WCSessionManager.store(session, peerId: peerId, peerMeta: .mock())
    }
    ...
    func testStore() {
        ...
        WCSessionManager.store(session, peerId: peerId, peerMeta: .mock())
        XCTAssertNotNil(WCSessionManager.load(session.topic))

        WCSessionManager.clear(session.topic)
        XCTAssertNil(WCSessionManager.load(session.topic))
    }

    func testLoad() {
        let topic = "f7fddc39-e4ab-4071-b51e-1b669244f516"
        let (session, peerId, peerMeta) = WCSessionManager.load(topic)!

        XCTAssertEqual(session.topic, topic)
        XCTAssertEqual(peerId, "779ee67f-6dbf-4c47-8589-16f1bcdd8e68")
        XCTAssertEqual(peerMeta.name, "WalletConnect Example")
        XCTAssertEqual(peerMeta.url, "https://example.walletconnect.org")
    }
    ...
    func testClearAll() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(2)) {
            WCSessionManager.clearAll()
            XCTAssertNil(WCSessionManager.load("f7fddc39-e4ab-4071-b51e-1b669244f516"))
            expect.fulfill()
        }
